...
async function getUrlsFor(pokemon) {
  ...
  const speciesUrl = species.url || `${pokeapi.BASE_URL}/pokemon-species/${species.id}/`;
  const speciesData = species.evolution_chain ? species : (await pokeapi.get(speciesUrl)).data;
...
// sync tries again.
export async function syncOfflineFavorites(favorites) {
