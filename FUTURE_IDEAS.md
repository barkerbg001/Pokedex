...
- [x] "Who's That Pokémon?" guessing game — show a darkened/silhouette sprite (CSS `brightness(0)` on the official artwork), let the user type or pick from multiple choice, reveal with the classic flip animation. Easy win: no new data needed, just a game mode over the existing sprite/name data. Shipped as a `quiz` view (`?view=quiz`, also a manifest shortcut) in [WhosThatPokemon.js](src/components/WhosThatPokemon/WhosThatPokemon.js): multiple choice (number keys 1–4) or a remembered "Type it" mode with forgiving matching ("mr. mime" = Mr-Mime), a pool of all generations or just one (built from the already-loaded generation species lists), score/streak with a saved best streak, the next round prefetched in the background, and "View in Pokédex" to open the revealed Pokémon's detail sheet. Reachable from the desktop sidebar and a Quiz tab that replaced Filter in the mobile bottom nav (the filter FAB now shows on mobile instead).
