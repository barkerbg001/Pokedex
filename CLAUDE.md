...
npx vitest run          # single non-watching run (use in CI/scripts)
npx vitest run src/components/Pokedex/Pokedex.favorites.test.tsx  # single test file
npm run test:coverage    # single run with a coverage report (text + html in coverage/)
npm run typecheck        # tsc --noEmit
npm run screenshots      # regenerate public/screenshots/*.png for the PWA manifest
npm run lint             # eslint .
npm run format           # prettier --write .
npm run format:check     # prettier --check . (no writes)
```

`vitest.config.ts` sets coverage thresholds (lines/statements/functions/branches) a little under actual coverage, as a regression floor rather than a target — raise them as more of the app gets covered.

## Architecture

### TypeScript throughout `src/`

The app is TypeScript: components are `.tsx`, non-JSX modules are `.ts`. Shared PokéAPI / app domain types live in [src/types/pokeapi.ts](src/types/pokeapi.ts). `tsc --noEmit` runs as part of `npm run build`. The service worker [public/sw.js](public/sw.js) stays plain JS (runs in the SW global scope, not the Vite bundle).

### Data layer: one API client, cached indefinitely

All PokéAPI access goes through [src/api/pokeapi.ts](src/api/pokeapi.ts) — a single axios instance with an in-memory `Map` cache keyed by URL. Because PokéAPI resources are immutable, responses are cached forever for the life of the tab (no TTL/eviction), and the cache stores the in-flight _promise_, not just the resolved value, so concurrent requests for the same URL are deduped automatically. Failed requests are retried twice with backoff and are not cached. `pokeapi.get<T>()` accepts either a path relative to the base URL or a full URL (as returned in other PokéAPI responses' `.url` fields), so callers rarely need to know which they have.
